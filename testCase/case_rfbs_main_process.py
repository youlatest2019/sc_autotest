...
# @Time     : 2022/8/8 16:19
# @Author   : yangshukun
# @File     : case_rfbs_main_process.py
import time
...
class RfbsMainProcessTest(InitCls, BsclMainKeyword, PubKeywordHdDataListPageForMx, PubKeywordHdPageForMx,
    ...
    def test_01_rfbs_main_process(self):
        ...
        response = self.queryBsrw(bsqj_date, "存量票据融资信息")
        if response:
        ...
        self.refreshSbDataForMx(start_date,end_date)
        self.tickAllBoxAllPageForMx()
    ...
    def test_02_rfbs_audit_refused(self):
        """金数--审批驳回后，核对页面可正常查看报送任务，且驳回意见正确"""
        mylog.info('\n************开始执行案例：【{}】************'.format(sys._getframe().f_code.co_name))
        # 1、登录(已在Init类中进行了登录，无需在案例中再次进行登录)
        # 2、进入报送管理主界面
        self.intoBsglMainMenu()
        self.switchToBsglBaseFrame()
        # 3、选择Rfbs品种进入Rfbs管理界面
        self.intoRfbsTab()
        # 4、切换到品种管理frame
        self.switchBspzFrame()
        # 5、进入报送范围管理页签
        ...
        self.refreshSpDataForMx()
        self.tickAllBoxAllPageForMx()
        self.inputAuditResonSpDataForMx("自动化Rfbs审批驳回")
        self.rejectSpDataForMx()
        res2 = self.GetTagText(self.MSG_PUB_MX)
        self.assertIn("审批成功", res2)
        self.clickAlertConfirmButtonForMx()
        # 13、点击主菜单--报送处理，进入Rfbs核对界面
        self.RefreshPage()
        self.clickBsclMainMenue()
        self.intoRfbsHdPage()
        # 14、输入报送期间，报送任务编号查询
        ...
        self.assertEqual("自动化Rfbs审批驳回", res3)
        mylog.info('\n************案例：【{}】执行结束************'.format(sys._getframe().f_code.co_name))

    def test_03_rfbs_go_back_task(self):
        """金数--上报页面退回上报数据，用户在核对页面可正常查看退回的任务，且退回意见正确"""
        mylog.info('\n************开始执行案例：【{}】************'.format(sys._getframe().f_code.co_name))
        # 1、登录(已在Init类中进行了登录，无需在案例中再次进行登录)
        # 2、进入报送管理主界面
        self.intoBsglMainMenu()
        ...
        # 退回全部分组子任务
        self.goBackAllGroupSbForMx("Rfbs自动化退回")
        # 15、点击主菜单--报送处理，进入Rfbs核对界面
        self.RefreshPage()
        self.clickBsclMainMenue()
        self.intoRfbsHdPage()
        # 16、选择数据日期，报送期间，输入报送任务编号查询
        self.choiceHdReportDateForMx(bsqj_date)
        self.inputHdReportNoForMx(report_no)
        self.refreshHdDataForMx()
        # 断言驳回成功，且驳回意见与审批一致
        res3 = self.GetElementAttrbute(self.DATA_PUB_REFUSE_RESON_MX, "value")
        self.assertEqual("Rfbs自动化退回", res3)
        mylog.info('\n************案例：【{}】执行结束************'.format(sys._getframe().f_code.co_name))
