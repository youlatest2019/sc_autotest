...
class ReastMainProcessTest(InitCls, BsclMainKeyword, PubKeywordHdDataListPageForMx, PubKeywordHdPageForMx,
    ...
    def test_01_reast_main_process(self):
        """EAST--建立分组->授权->新增报送任务-->核对提交->审批通过->确认成功->查询，主流程案例"""
        ...
        # 6、新增分组
        group_name = "reast自动化分组" + get_now_time_to_str()
        self.addGroup(group_name, user_chinese_name, "自动化测试_reast分组描述")
        # 7、分组授权
        ...
        response = self.queryBsrw(bsqj_date, "员工信息")
        if response:
            res = self.GetTagText(self.COUNG_QUERY_RESULT_BSRW)
            if res != "共有 0 个报送任务，0 个已过期":
                self.abandonBsrw("reast任务自动化废弃")
        # 17、新增报送任务
        self.intoBsglMainMenu()
        self.switchToBsglBaseFrame()
        # 18、选择EAST品种进入EAST管理界面
        self.intoReastTab()
        self.switchBspzFrame()
        self.swithToBsrw()
        self.switchBspzFrame()
        self.addBsrw(bsnr_no, bsqj_no, qssd_index=3, type="season", bsqj_month="第二季度")
        self.addBsrwSave()
        # 19、点击主菜单 - -报送处理，进入reast核对界面
        self.RefreshPage()
        self.clickBsclMainMenue()
        self.intoReastHdPage()
        # 20、输入报送期间，报送任务编号查询
        ...
        self.RefreshPage()
        self.clickBsclMainMenue()
        self.intoReastSbPage()
        # 25、搜索对应报送任务，全选，不下载，确认成功
    ...
    def test_02_reast_audit_refused(self):
        """east--审批驳回后，核对页面可正常查看报送任务，且驳回意见正确"""
        mylog.info('\n************开始执行案例：【{}】************'.format(sys._getframe().f_code.co_name))
        # 1、登录(已在Init类中进行了登录，无需在案例中再次进行登录)
        # 2、进入报送管理主界面
        self.intoBsglMainMenu()
        ...
        self.refreshSpDataForMx()
        self.tickAllBoxAllPageForMx()
        self.inputAuditResonSpDataForMx("自动化reast审批驳回")
        self.rejectSpDataForMx()
        res2 = self.GetTagText(self.MSG_PUB_MX)
        self.assertIn("审批成功", res2)
        self.clickAlertConfirmButtonForMx()
        # 13、点击主菜单--报送处理，进入reast核对界面
        self.RefreshPage()
        self.clickBsclMainMenue()
        self.intoReastHdPage()
        # 14、输入报送期间，报送任务编号查询
        ...
        self.assertEqual("自动化reast审批驳回", res3)
        mylog.info('\n************案例：【{}】执行结束************'.format(sys._getframe().f_code.co_name))

    def test_03_reast_go_back_task(self):
        """east--上报页面退回上报数据，用户在核对页面可正常查看退回的任务，且退回意见正确"""
        mylog.info('\n************开始执行案例：【{}】************'.format(sys._getframe().f_code.co_name))
        # 1、登录(已在Init类中进行了登录，无需在案例中再次进行登录)
        # 2、进入报送管理主界面
        self.intoBsglMainMenu()
        ...
        # 退回全部分组子任务
        self.goBackAllGroupSbForMx("reast自动化退回")
        # 15、点击主菜单--报送处理，进入reast核对界面
        self.RefreshPage()
        self.clickBsclMainMenue()
        self.intoReastHdPage()
        # 16、输入报送期间，报送任务编号查询
        ...
        # 断言驳回成功，且驳回意见与审批一致
        res3 = self.GetElementAttrbute(self.DATA_PUB_REFUSE_RESON_MX, "value")
        self.assertEqual("reast自动化退回", res3)
        mylog.info('\n************案例：【{}】执行结束************'.format(sys._getframe().f_code.co_name))
