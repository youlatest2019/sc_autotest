...
class YwblKeyword(WebDriver, YwblElementMain, YwblElementAdd, YwblElementFrame, YwblElementDataPath, DataYgxx):
    ...
    def importData(self, file_dir):
        """导入数据"""
        base_dir = os.path.join(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data'), '补录')
        file_dir_name = os.path.split(file_dir)
        abs_path = os.path.join(base_dir, file_dir_name[0], file_dir_name[1])
        self.Click(self.BUTTON_DR)
        self.SwitchFrame(self.FRAME_DR_INDEX)
        self.Click(self.BUTTON_UPLOAD)
        self.GotoSleep(1)
        self.UploadFile(abs_path)
    ...
    def commitAllDataForHl(self):
        """汇率模型特殊提交数据"""
        self.Click(self.BUTTON_TJQB)
        self.GotoSleep(0.5)
        self.Click(self.ALTER_TJQB_HL)
        self.GotoSleep(3)
