...
class BsglRptcTest(InitCls, BsglMainKeyword, OperationXml, BsglPubBsrwKeyword, BsglPubBsfwKeyword,
                   ...
                   BsglPubHswhKeyword, BsglPubSjxwhKeyword, ZdysjlxPubSplcKeyword, XsjdpzPubSplcKeyword,
                   PubLsbbKeyword):
    ...
    def test_03_add_bsrw_rptc_success(self):
        ...
        self.switchBspzFrame()
        # 判断报送任务是否存在，存在则废弃
        response = self.queryBsrw("2021年上半年", "T115-绿色融资统计表")
        if response:
    ...
    def test_16_rptc_xsjdpz_change_success(self):
        """rptc小数精度配置--配置小数精度成功"""
        mylog.info('\n************开始执行案例：【{}】************'.format(sys._getframe().f_code.co_name))
        # 1、登录(已在Init类中进行了登录，无需在案例中再次进行登录)
        # 2、进入报送管理主界面
        ...
        # 13、输入报送内容名称
        self.inputXsjdpzBsnrmc("FXCJGZB01")
        # 14、刷新页面
    ...
    def test_17_import_lsmodule_success(self):
        ...
        self.changeLsbbImport(self.PHJR01_PATH)
        self.GotoSleep(3)
        # 断言导入成功
        res = self.GetTagText(self.MSG_ADD_RESULT)
