            ...
            # 处理应用包中的独立服务，属于特殊应用，包括文件目录，jar包，不同的sql目录
                ...
                for f in base_files_list:
                    if f == 'config':
                        ...
                        lib_source_path = dcab_app_source_path + r'\{}'.format(f)
                        lib_target_path = self.base_target_dir + r'\{}\bs\dcab-app\lib'.format(pt_package_name)
                        if os.path.exists(lib_target_path):
                            shutil.rmtree(lib_target_path)
                        shutil.copytree(lib_source_path, lib_target_path)

                    elif f.startswith('start'):
                        shell_source_path = dcab_app_source_path + r'\{}'.format(f)
                        shell_target_path = self.base_target_dir + r'\{}\bs\dcab-app\{}'.format(pt_package_name, f)
                        if os.path.exists(shell_target_path):
                            os.remove(shell_target_path)
                        shutil.copy(shell_source_path, shell_target_path)
...
if __name__ == '__main__':
    a = MakeInstallPackage(r'D:\应用打包', r'D:\数仓安装记录\test\新建文件夹')
