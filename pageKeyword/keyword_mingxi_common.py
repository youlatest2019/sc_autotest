...
class PubKeywordHdPageForMx(WebDriver, PubMxBspzElement):  # 明细类核对页面公共关键字
    ...
    def tickOneBoxByIndexForMx(self, index_no):
        """根据数据行索引勾选数据"""
        self.Click(self.BOX_ONE_PUB_MX.format(index_no))

    def clickAlertConfirmButtonForMx(self):
        """报表类品种--通用弹窗确认按钮（提交、审批同意、驳回）"""
        self.Click(self.ALERT_PUB_MX)

    def get_data_of_check_succ(self, range_no):
        """随机获取一条校验通过的数据"""
        for index_no in range(1, range_no + 1):
            img_title = self.GetElementAttrbute(self.IMG_PUB_HD_CHECK_FLAG_MX.format(index_no), "title")
            if img_title == "校验通过":
                return index_no
            else:
                continue
...
class PubKeywordSbPageForMx(WebDriver, PubMxBspzElement):
    ...
    def downLoadSbDataForMx1(self):
        """明细类品种--上报页面--下载报文"""
        self.Click(self.BUTTON_PUB_SB_DOWN_LOAD_MX1)
    ...
    def confirmSuccessSbForMx1(self):
        """明细类品种--上报页面--确认成功"""
        self.Click(self.BUTTON_PUB_SB_QRCG_MX1)
    ...
    def intoReportInformationSbForMx(self):
        """明细类品种--上报页面--进入报送任务详情"""
        self.Click(self.BUTTON_PUB_SB_RWXQ_MX)

    def goBackSbForMx(self):
        """明细类品种--上报页面--进入报送任务详情--退回按钮"""
        self.Click(self.BUTTON_PUB_SB_BACK_MX)

    def changeToBackFrameSbForMx(self):
        """明细类品种--上报页面--进入报送任务详情--退回--切换到退回窗口frame"""
        self.SwitchFrameByElement(self.FRAME_PUB_SB_BACK_MX)

    def tickBackBoxSbForMx(self):
        """明细类品种--上报页面--进入报送任务详情--退回窗口-全选"""
        self.Click(self.BOX_PUB_SB_BACK_ALL_MX)

    def goBackOfAlertSbForMx(self):
        """明细类品种--上报页面--进入报送任务详情--退回窗口--退回按钮"""
        self.Click(self.BUTTON_PUB_SB_BATCH_BACK_MX)

    def goBackAllGroupSbForMx(self, back_reson):
        """明细类品种--上报页面--进入报送任务详情--退回窗口--退回原因窗口--填写退回原因"""
        self.Click(self.BUTTON_PUB_SB_BACK_MX)
        self.SwitchFrameByElement(self.FRAME_PUB_SB_BACK_MX)
        self.Click(self.BOX_PUB_SB_BACK_ALL_MX)
        self.Click(self.BUTTON_PUB_SB_BATCH_BACK_MX)
        self.QuiteFrame()
        self.SwitchFrameByElement(self.FRAME_PUB_SB_BACK_RESON_ALERT_MX)
        self.Click(self.INPUT_PUB_SB_BACK_RESON_ALERT_MX)
        self.Input(back_reson, self.INPUT_PUB_SB_BACK_RESON_ALERT_MX)
        self.Click(self.BUTTON_PUB_SB_BACK_RESON_ALERT_CONFIRM_MX)
...
class PubKeywordCxPageForMx(WebDriver, PubMxBspzElement):
    ...
    def downLoadCxDataForMx(self):
        """明细类品种--查询页面--批量导出(excel)"""
        self.Click(self.BUTTON_PUB_CX_DOWN_LOAD_MX)

    def inputCxBusinessName(self):
        """征信数据更正-企业名称输入"""
        self.Input()
